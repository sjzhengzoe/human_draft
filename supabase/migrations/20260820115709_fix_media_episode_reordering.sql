...
begin
  ...
  loop
    ...
    loop
      ...
      if char_length(draft_plot_summary) > 20 then
        raise exception using errcode = '22023', message = '剧情详情不能超过 20 个字';
      end if;
  ...
  loop
    ...
    -- Existing episode numbers are unique within a season. Move retained rows
    -- out of the final 1..500 range before assigning the requested order so a
    -- swap never collides with a row that has not been renumbered yet.
    ...
    loop
      ...
      if nullif(episode_item->>'id', '') is null then
        insert into public.media_episodes(
          uid, season_id, episode_number, plot_summary, is_favorite
        ) values (
          p_uid,
          saved_season_id,
          episode_position,
