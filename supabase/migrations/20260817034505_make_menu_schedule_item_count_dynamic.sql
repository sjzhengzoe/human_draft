-- A meal now stores only its selected items. The former slot count created
-- artificial empty cards and capped a meal at twelve selections.

begin;

do $$
declare
  has_invalid_item_count boolean := false;
begin
  if exists (
    select 1 from information_schema.columns
    where table_schema = 'public'
      and table_name = 'menu_schedule_meals'
      and column_name = 'slot_count'
  ) then
    execute $audit$
      select exists (
        select 1
        from public.menu_schedule_meals meal
        where meal.slot_count < (
          select count(*)
          from public.menu_schedule_items item
          where item.meal_id = meal.id
        )
      )
    $audit$ into has_invalid_item_count;
  end if;
  if has_invalid_item_count then
    raise exception 'MENU_SCHEDULE_ITEM_COUNT_INVALID';
  end if;
end;
$$;

drop function if exists public.replace_menu_schedule_meal(text, date, text, integer, jsonb);

alter table public.menu_schedule_meals
  drop constraint if exists menu_schedule_meals_slot_count_check;
...
begin
  ...
  insert into public.menu_schedule_meals (
    uid, meal_date, meal_period
  ) values (
    p_uid, p_meal_date, p_meal_period
  )
...
revoke all on function public.replace_menu_schedule_meal(text, date, text, jsonb)
  from public, anon, authenticated;
grant execute on function public.replace_menu_schedule_meal(text, date, text, jsonb)
  to service_role;
