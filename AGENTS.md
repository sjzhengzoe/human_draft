...
- A short edit containing one or two closely related fields must use `app-dialog placement="bottom"`. Bottom dialogs must let `app-dialog` own keyboard-height movement; their inputs use `dialog-mode` or `adjust-position="{{false}}"` and must not add page-level keyboard spacers.
- Editing that combines images, dates, long text, multiple independent fields, or item management must use a dedicated page. Do not compress these workflows into either a centered or bottom dialog.
