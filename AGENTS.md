...
- A standalone short edit containing one or two closely related fields must use `app-dialog placement="bottom"`. Treat fields with a maximum length of about 120 characters or less as short by default. Bottom dialogs must let `app-dialog` own keyboard-height movement; their inputs use `dialog-mode` or `adjust-position="{{false}}"` and must not add page-level keyboard spacers.
