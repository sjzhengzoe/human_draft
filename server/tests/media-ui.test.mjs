...
test("media cards show titles, watch progress, record ratings, and category-specific placeholders", async () => {
  ...
  assert.match(logic, /const seasonNumber = Number\.isInteger\(entry\.last_watched_season_number\)/);
  assert.match(logic, /return `看到第 \$\{seasonNumber\} 季 · \$\{entry\.last_watched_episode_number\} 集`/);
  assert.match(logic, /return `看到 \$\{entry\.last_watched_episode_number\} 集`/);
  assert.doesNotMatch(logic, /last_watched_season_name}·/);
...
test("media detail embeds a ranged episode picker with short plot summaries", async () => {
  ...
  assert.match(page, /class="detail-watch-progress__value"[\s\S]*?<app-icon name="chevron-right" size="16"/);
  assert.match(page, /episodeRangeOptions\.length > 1 && !episodePickerFavoriteOnly/);
  assert.match(page, /class="episode-picker__range-scroll"[\s\S]*?scroll-x[\s\S]*?scroll-into-view="episode-range-\{\{activeEpisodeRangeIndex\}\}"/);
  assert.match(page, /wx:for="\{\{episodeRangeOptions\}\}"[\s\S]*?class="episode-picker__range-option/);
  assert.match(page, /episode-picker__range-option--active/);
  assert.doesNotMatch(page, /快速选集|展开全部集数范围|episode-picker__range-expand/);
  assert.match(page, /wx:for="\{\{episodePickerEpisodes\}\}"/);
  assert.match(page, /class="episode-picker__list"/);
  assert.match(page, /class="episode-row__media-icon"[\s\S]*?name="\{\{isAudio \? 'headphones' : 'clapperboard'\}\}"/);
  assert.match(page, /class="episode-row__title">\{\{item\.title \|\| '第 '/);
  assert.doesNotMatch(page, /episode-picker__tab-count|\{\{item\.episodes\.length\}\} 集/);
  assert.match(page, /\{\{item\.plot_summary \|\| '点击添加剧情详情'\}\}/);
  assert.match(page, /catchtap="handleFavoriteTap"/);
  assert.match(page, /wx:if="\{\{activeSeasonFavoriteCount > 0\}\}"[\s\S]*?bindtap="handleEpisodePickerFavoriteTap"/);
  assert.match(logic, /episodePickerFavoriteOnly = this\.data\.episodePickerFavoriteOnly[\s\S]*?&& activeSeasonFavoriteCount > 0/);
  assert.match(page, /bindtap="handleEpisodeSummaryTap"/);
  assert.match(page, /placement="bottom"[\s\S]*?maxlength="\{\{textSheetMaxlength\}\}"/);
  assert.match(page, /textSheetPurpose === 'episode-summary'[\s\S]*?\{\{textSheetValue\.length\}\} \/ \{\{textSheetMaxlength\}\}/);
  assert.match(logic, /const EPISODE_RANGE_SIZE = 10/);
  assert.match(logic, /Math\.ceil\(episodeCount \/ EPISODE_RANGE_SIZE\)/);
  assert.match(logic, /if \(favoriteOnly\) return season\.episodes\.filter/);
  assert.doesNotMatch(logic, /handleEpisodeRangeDialog|episodeRangeDialogVisible/);
  assert.match(logic, /handleEpisodeSummaryTap/);
  assert.match(logic, /EPISODE_SUMMARY_MAX_LENGTH = 24/);
  assert.match(logic, /textSheetMaxlength: EPISODE_SUMMARY_MAX_LENGTH/);
  assert.match(logic, /剧情详情不能超过 \$\{EPISODE_SUMMARY_MAX_LENGTH\} 个字/);
  ...
  assert.match(page, /class="detail-minimal__heading"[\s\S]*?class="detail-minimal__title"[\s\S]*?class="special-favorite-toggle/);
  assert.doesNotMatch(page, /detail-action-row|detail-title-actions/);
  ...
  assert.doesNotMatch(minimalDetails, /chevron-right|name="pencil"/);
  ...
  assert.match(styles, /\.episode-picker\s*\{[^}]*margin-top:\s*20rpx;[^}]*padding-top:\s*20rpx;[^}]*border-top:\s*1rpx solid var\(--ui-color-border\);/s);
  assert.doesNotMatch(styles, /\.detail-watch-progress\s*\{[^}]*border-(?:top|bottom):/s);
  assert.match(styles, /\.detail-watch-progress\s*\{[^}]*min-height:\s*64rpx;[^}]*margin-top:\s*0;/s);
  assert.match(styles, /\.detail-minimal\s*\{[^}]*padding:\s*18rpx 0 0;/s);
  assert.match(styles, /\.episode-picker__list\s*\{[^}]*margin-top:/s);
  assert.match(styles, /\.episode-row\s*\{[^}]*display:\s*grid;[^}]*grid-template-columns:/s);
  assert.doesNotMatch(styles, /\.episode-card\s*\{/);
  ...
  assert.match(styles, /\.episode-picker__tab\s*\{[^}]*min-height:\s*56rpx;[^}]*padding:\s*0 20rpx;/s);
  assert.doesNotMatch(styles, /\.episode-picker__tab-count\s*\{/);
  assert.doesNotMatch(styles, /episode-range-dialog|episode-picker__range-expand/);
  assert.match(styles, /\.episode-row__favorite\s*\{[^}]*width:\s*56rpx;[^}]*height:\s*56rpx;/s);
});
...
test("media watch progress uses one shared descending picker on lists and details", async () => {
  ...
  assert.match(detailLogic, /const seasonNumber = Number\.isInteger\(entry\.last_watched_season_number\)/);
...
test("season management uses one accordion draft page and saves once", async () => {
  ...
  assert.match(page, /expandedSeasonKey === item\.key \? 'season-card--expanded' : ''/);
  assert.match(page, /bindtap="handleSeasonToggle"/);
  assert.match(page, /episode-editor-row__summary[\s\S]*?\{\{episode\.plot_summary \|\| '点击添加剧情详情'\}\}/);
  assert.match(page, /episode-editor-row__summary[^>]*bindtap="handleEpisodeSummaryTap"/);
  assert.match(page, /<app-field-editor[\s\S]*?title="\{\{fieldEditorTitle\}\}"[\s\S]*?type="\{\{fieldEditorType\}\}"[\s\S]*?bind:confirm="handleFieldEditorConfirm"/);
  assert.match(logic, /EPISODE_SUMMARY_MAX_LENGTH = 24/);
  assert.match(logic, /handleEpisodeSummaryTap[\s\S]*?purpose: "episode-summary"/);
  assert.match(logic, /handleFieldEditorConfirm[\s\S]*?purpose === "episode-summary"[\s\S]*?plot_summary`\]: value/);
  assert.match(page, /season-field[^>]*bindtap="handleSeasonNameTap"[\s\S]*?season-field__value/);
  assert.match(page, /season-field[^>]*bindtap="handleEpisodeCountTap"[\s\S]*?\{\{item\.episodes\.length\}\} 集/);
  assert.doesNotMatch(page, /<app-input|episode-count-editor/);
  ...
  assert.match(page, /season-card__drag-handle[\s\S]*?name="drag-handle"/);
  assert.match(page, /episode-editor-row__drag-handle[\s\S]*?name="drag-handle"/);
  assert.doesNotMatch(page, /episode-editor-row__media-icon|name="clapperboard"/);
  assert.match(page, /episode-editor-row__title[\s\S]*?bindtap="handleEpisodeTitleTap"[\s\S]*?\{\{episode\.title \|\| '第 '/);
  assert.match(logic, /handleEpisodeTitleTap[\s\S]*?purpose: "episode-title"[\s\S]*?title: "修改单集名称"/);
  assert.match(page, /episode-editor-row__copy[\s\S]*?episode-editor-row__title[\s\S]*?episode-editor-row__summary/);
  assert.match(page, /<sort-drag-ghost/);
  assert.doesNotMatch(page, /name="move"/);
  assert.match(page, /catchtouchmove="handleSortDragMove"/);
  assert.match(page, /scroll-y="\{\{draggingSortKey === ''\}\}" scroll-top="\{\{managerScrollTop\}\}"/);
  ...
  assert.match(logic, /createDragSortController/);
  assert.doesNotMatch(logic, /dragLongPressTimer|pendingSortDrag/);
  assert.match(logic, /seasonDragSort\.start/);
  assert.match(logic, /seasonDragSort\.move/);
  assert.match(logic, /seasonDragSort\.finish/);
  assert.match(page, /style="\{\{dragSortKind === 'season' \? dragSortPreviewStyles\[index\] : ''\}\}"/);
  assert.match(page, /style="\{\{dragSortKind === 'episode' \? dragSortPreviewStyles\[episodeIndex\] : ''\}\}"/);
  assert.match(logic, /handleSortDragEnd[\s\S]*?seasonDragSort\.finish\(this, this\.data\.draftSeasons/);
  assert.match(logic, /draftSeasons: result\.items,[\s\S]*?dirty: true/);
  assert.doesNotMatch(logic, /reorderByDrop|drag\.targetKey/);
  assert.doesNotMatch(logic, /startSeasonSortDrag|startEpisodeSortDrag/);
  assert.doesNotMatch(logic, /dragTargetIndex|dragInsertAfter/);
  assert.match(logic, /scheduleDragAutoScroll/);
  assert.match(logic, /seasonDragSort\.adjustForScroll/);
  assert.doesNotMatch(page, /drop-before|drop-after/);
  assert.doesNotMatch(styles, /drop-before|drop-after|will-change/);
  assert.match(styles, /\.episode-editor-row--dragging\s*\{[^}]*opacity:\s*0;/s);
  ...
  assert.match(styles, /\.season-card--expanded \.season-card__header\s*\{[^}]*box-shadow:\s*inset 6rpx 0 var\(--ui-color-action-primary\)/s);
  assert.doesNotMatch(styles, /\.season-card--expanded \.season-card__header\s*\{[^}]*background:/s);
  assert.match(styles, /\.season-editor__fields\s*\{[^}]*display:\s*flex;[^}]*flex-direction:\s*column;/s);
  assert.match(styles, /\.episode-editor-row\s*\{[^}]*grid-template-columns:\s*56rpx minmax\(0, 1fr\) 56rpx;[^}]*border-bottom:/s);
...
test("media UI keeps dense controls compact while improving long-list interactions", async () => {
  ...
  assert.match(detailPage, /name="\{\{item <= \(entry\.personal_rating \|\| 3\) \? 'star-rating-filled' : 'star-muted'\}\}" size="30"/);
  ...
  assert.match(detailLogic, /EPISODE_RANGE_SIZE = 10/);
  assert.match(detailLogic, /episodePickerEpisodes: episodePickerEpisodes/);
  assert.match(detailStyles, /\.episode-row\s*\{[^}]*display:\s*grid;/s);
});
