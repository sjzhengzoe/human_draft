...
test("weekly menu component keeps focused day and week displays with the random flow", async () => {
  ...
  assert.match(style, /\.meal-section \{[^}]*border: 1rpx solid var\(--ui-color-border\)[^}]*border-radius: 20rpx[^}]*background: var\(--ui-color-background-surface\)/);
  assert.match(style, /\.meal-list \{[^}]*flex-direction: column[^}]*gap: 24rpx/);
  assert.match(style, /\.meal-section__header \{[^}]*min-height: 42rpx[^}]*padding: 0 20rpx/);
  ...
  assert.match(style, /\.meal-slot \{[^}]*width: 176rpx[^}]*height: 210rpx/);
  assert.match(style, /\.meal-slot--add \{[^}]*width: 176rpx[^}]*height: 210rpx/);
  assert.match(style, /\.meal-slot__name \{[^}]*height: 76rpx/);
